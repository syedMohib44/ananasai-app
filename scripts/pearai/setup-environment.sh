...
app_dir=$(pwd)
target_path="$app_dir/extensions/ananasai-submodule/extensions/vscode"
link_path="$app_dir/extensions/ananasai-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/ananasai-submodule" ]; then
    echo "Removing existing ananasai-submodule directory"
    execute "rm -rf $app_dir/extensions/ananasai-submodule" "Failed to remove existing ananasai-submodule directory"
fi
...
execute "cd ./extensions/ananasai-submodule" "Failed to change directory to extensions/ananasai-submodule"
