	...
	function moveCopyActiveEditorToGroup(isMove: boolean, args: ActiveEditorMoveCopyArguments, control: IVisibleEditorPane, accessor: ServicesAccessor): void {
		...
		switch (args.to) {
			...
			case 'next':
				...
				break;
			case 'center':
				targetGroup = editorGroupService.getGroups(GroupsOrder.GRID_APananasANCE)[(editorGroupService.count / 2) - 1];
				break;
			case 'position':
				targetGroup = editorGroupService.getGroups(GroupsOrder.GRID_APananasANCE)[(args.value ?? 1) - 1];
				break;
		}
